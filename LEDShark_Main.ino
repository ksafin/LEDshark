...
#define STRIP1_SIG 19
#define STRIP2_SIG 18

#define BASS_SCALER 1.5
#define MID_SCALER 8
#define HIGH_SCALER 5

#define MIN_BRIGHTNESS 20

bool rearEnabled = true;               // Whether rear LED's are enabled
long switchStart = 0;
...
const int AUDIO_INPUT_PIN = R_NORM2;        // Input ADC pin for audio data.
...
const int STRIP1_COUNT = 140;         // Number of neo pixels.  You should be able to increase this without
const int STRIP2_PIN = STRIP2_SIG;
const int STRIP2_COUNT = 50;       // Number of neo pixels.  You should be able to increase this without
...
#define maxMid 60000
#define maxHigh 80000

//60 dank, just not bright enough

//30k all for 5 scaler
...
void blankStrip(int strip) {
  ...
  if(strip == 0) {
    ...
    for(int i = 0; i < STRIP2_COUNT; i++) {
      setRearStrip(i, strip2.Color(0, 0, 0));
    }
  }
}

void setRearStrip(int idx, uint32_t color) {
  if(rearEnabled) {
    strip2.setPixelColor(idx, color);
  }
}
...
void setRangeToColor(int start, int end, int brightness, int color, int strip) {
  ...
  if(strip == 0) {
    ...
    for(int i = start; i < end; i++) {
      ...
      setRearStrip(i, getColor(brightness, color));
    }
  }
...
float getHighMag() {
  ...
  return highestInRange(highStartBucket, highEndBucket);
  //return magnitudes[frequencyToBin(4000)];
}
...
void setup() {
  ...
  pinMode(LV_EN, OUTPUT);
  digitalWrite(LV_EN, HIGH);
  
  pinMode(R_NORM1, INPUT);
  pinMode(R_NORM2, INPUT);
  Serial.begin(115200);

  pinMode(MODE, INPUT);
  attachInterrupt(MODE, mode_low, FALLING);
  attachInterrupt(MODE, mode_high, RISING);

  bassBin = frequencyToBin(60.0);
  midStartBucket = frequencyToBin(300.0);
  midEndBucket = frequencyToBin(1000.0);
...
void mode_low() {
  long elapsed = millis() - switchStart;
  if(elapsed > 2000) rearEnabled = !rearEnabled;
